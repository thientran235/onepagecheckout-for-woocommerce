	...
	class WC_City_Select {
		...
		public function form_field_city( $field, $key, $args, $value ) {
			...
			if ( $args['required'] ) {
				...
				$required = ' <abbr class="required" title="' . esc_attr__( 'required', 'wcvn-onepagecheckout'  ) . '">*</abbr>';
			} else {
				$required = '';
			}
			...
			if ( is_array( $cities ) ) {
				...
				$field .= '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="city_select ' . esc_attr( implode( ' ', $args['input_class'] ) ) .'" ' . implode( ' ', $custom_attributes ) . ' placeholder="' . esc_attr( $args['placeholder'] ) . '">
					<option value="">'. __( 'Select an option&hellip;', 'wcvn-onepagecheckout' ) .'</option>';
		...
		public function load_scripts() {
			if ( is_cart() || is_checkout() || is_wc_endpoint_url( 'edit-address' ) ) {
				...
				wp_enqueue_script( 'wc-city-select', $city_select_path, array( 'jquery', 'wcvn-onepagecheckout' ), self::VERSION, true );

				$cities = json_encode( $this->get_cities() );
				wp_localize_script( 'wc-city-select', 'wc_city_select_params', array(
					'cities' => $cities,
					'i18n_select_city_text' => esc_attr__( 'Select an option&hellip;', 'wcvn-onepagecheckout' )
				) );
			}
		}
